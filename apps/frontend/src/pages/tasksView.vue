  ...
  <div class="min-h-screen bg-gradient-to-b from-gray-100 to-gray-50 p-6">
    ...
    <div class="overflow-x-auto rounded-lg shadow-lg">
      <table class="min-w-full bg-white rounded-lg">
        ...
        <tbody>
          <tr v-for="task in tasksStore.tasks" :key="task.id" class="border-b hover:bg-gray-50 transition">
            <td class="px-6 py-4 text-gray-700 font-medium">{{ task.titulo }}</td>
            <td class="px-6 py-4 text-gray-600">{{ task.descripcion }}</td>
            <td class="px-6 py-4">
              <span 
                :class="{
                  'text-yellow-600 bg-yellow-100 px-2 py-1 rounded-full': task.estado === 'Pendiente',
                  'text-blue-600 bg-blue-100 px-2 py-1 rounded-full': task.estado === 'En Progreso',
                  'text-green-600 bg-green-100 px-2 py-1 rounded-full': task.estado === 'Completada'
                }"
